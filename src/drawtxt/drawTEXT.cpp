...
/*  Brandon Esquivel Molina
    ...
    THIS IS A FIRST TEST IMPLEMENTATION OF THE ORIGINAL CODE (on dev)
...
{   
    ...
    image = IMG_Load("prueba.png");             // loads image, just change de extension .png or .pnm
    ...
    window = SDL_CreateWindow("SDL2 Voronoi PNG", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, 0);       // create a window, first layer
    ...
    int NP;         // this is just a test, real values come from a file read. skip
    vector<int> Sites;
    Sites.push_back(0);
    Sites.push_back(50);
    Sites.push_back(0);
    ...
    {
        (Sites[i]>maxX) ? maxX=Sites[i] : maxX=maxX;
    }
     for (int i = 0; i < Sites.size(); i = i+2)
    {
        (Sites[i]<minX) ? minX=Sites[i] : minX=minX;
    }

    for (int i = 1; i < Sites.size(); i = i+2)
    {
        (Sites[i]>maxY) ? maxY=Sites[i] : maxY=maxY;
    }
     for (int i = 1; i < Sites.size(); i = i+2)
    {
        (Sites[i]<minY) ? minY=Sites[i] : minY=minY;
    }
    ...
    {   
        const char * c = xtexty[i].c_str();             // Cast string to const Char, needed for the Render Text process
        ...
        dstrec.x = w/2 + Sites[j]*kx - Sites[j];            // start point xy, applying scaling
        dstrec.y = h/2 - Sites[j+1]*ky + Sites[j+1];
